...
#' todo: put this code back in tvvarss package where it belongs
#'
...
#' @param scalevariance [optional] Matrix of asymptotic, or stationary, variance to scale the
#' process errors to.
#'
...
simTVVAR <- function(Bt, topo=NULL, TT, var_QX, cov_QX, var_QB, cov_QB = 0,
  ...
  if (is.null(QQ_XX)) {
    ...
    # KFJ: do you really want this constraint? Wouldn't you be able
    # to specify a scalar for the diag and a different scalar for the
    # off-diagonals, so
    # length(var_QX) >= 1 & length(cov_QX) != 1
    # Or, just test that the length of the cov_QX should always be 1
    # and that the length of var_QX should be one or nn
  ...
  if (is.null(QQ_BB)) {
    ...
    if (!all(dim(QQ_BB) == nn * nn)) {
      # KFJ: why do you make it an nn^2 x nn^2 matrix rather than as
      # specified below?
  ...
  if(is.null(topo)) { ## matrix constrained 0:1 on diagonal, and -1:1 elsewhere
    ...
    diag(topo) <- "dd"
    # KFJ: should also find negative values and specify what type of interaction
    # they are
  }
  ...
  if (length(dim(Bt)) == 3) { ## then Bt is [n x n x T] array of interactions
    BB <- Bt
    # KFJ: why are process errors removed if Bt is time varying?
    WW_BB <- NULL
  }
  # Perform optional scaling
  if (!is.null(scalevariance)) {
    if (nrow(scalevariance) != ncol(scalevariance)) stop("The matrix provided ",
      "to scale the variance is a ", nrow(scalevariance), " by ",
      ncol(scalevariance), " matrix and should be square.")
    if (length(dim(Bt)) == 3) stop("Cannot perform scaling if",
      " B is time varying.")
    QQ_XX <- scale2var(B = BB[, , 1], varinf = scalevariance, vare = NULL)
  }

  ## proc errors for states
  eig <- eigen(QQ_XX)$values
  if (any(sapply(eig, class) == "complex")) stop("There are complex numbers",
    " in the eigen values of the process error matrix.")
  if (!all(eig >= 0)) stop("The matrix for process error ",
    "covariance is not positive definite")
  WW_XX <- t(MASS::mvrnorm(TT, matrix(0, nn, 1), QQ_XX))
  WW_inf <- scale2var(B = BB[, , 1], varinf = NULL, vare = QQ_XX)
  ...
  for (t in 1:TT + 1) {
    ...
    if (length(dim(Bt)) != 3) {
      ...
      # KFJ: This doesn't deal with turning negatives into positives,
      # instead it gives an error. As written, only density dependence
      # in the diagonal is allowed, not negative autocorrelation.
...
#' Scale the variance of a multivariate time series
#'
#' @details From Ives \emph{et al}. (2003) Equation 17.
#' The variance matrix will be used to solve for the other
#' matrix, where both entries cannot be \code{NULL}. One always needs
#' to specify the \code{\strong{B}} matrix.
#' @param B A square matrix of interactions.
#' @param varinf A square matrix of overall variance on
#' the diagonal and the covariance in the off diagonal
#' that will be used to solve for the process error of the
#' time series given the stationary distribution.
#' @param vare A square matrix of process errors that
#' will be used to solve for the stationary variance
#' of the time series if you had an infinite sample size.
#' @author Kelli Faye Johnson
#' @examples
#' scale2var(varinf = matrix(c(1, 0, 0, 1), nrow = 2, ncol = 2),
#'   B = matrix(c(-0.9, 0.1, 0.1, 0.9), nrow = 2))

scale2var <- function(B, varinf = NULL, vare = NULL) {
  # todo: ask about the identity matrix of B or B^2

  myvec <- function(data) {
    matrix(data, ncol = 1)
  }

  if (is.null(varinf) & is.null(vare)) stop("varinf and ",
    "vare cannot both be NULL")

  nn <- nrow(B)

  # Code from EW
  # vecVinf = solve(diag(nrow(B)) - kronecker(B,B)) %*%
  # matrix(c(Sigma), ncol=1)

  middle <- diag(nn^2) - kronecker(B, B)
  if (!is.null(vare)) {
    out <- tryCatch(solve(middle), error = function(e) e)
    if(any(class(out) == "error")) return(out)
    side <- out %*% myvec(vare)
  }
  if (!is.null(varinf)) {
    side <- (middle) %*% myvec(varinf)
  }
  matrix(c(matrix(c(side), ncol = 1)),
    ncol = nn, nrow = nn)
}
